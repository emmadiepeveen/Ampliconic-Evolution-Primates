...
│   └── 05_intraspecific_variation/   # Estimation of copy numbers across individuals of the same species 
...
### 5. Intraspecific variation in copy number (`scripts/05_intraspecific_variation/`)

Identifies copies of 7 X-linked ampliconic families in chimpanzee, human and gorilla from long read denovo assemblies. 

**Key scripts:**
- `wfmash.sh.sh` - Identifies all X-linked contigs
- `Intraspecific_variation.ipynb` - finds copy number per de novo assembly + estimates pN/pS per family per individual + makes gene trees per species per family

**Outputs used in figures:**
- Copy number variation across individuals → Figure 6
- pN/pS estimates per individual per family → Supplementary Table 12
- Gene trees per species per family -> Supplementary Figure 8-11
...
- Long-read de novo assemblies from Chimpanzees, Humans, and Gorillas are from Porsborg et al., 2025 (DOI: https://doi.org/10.1038/s41467-025-65248-3) 
